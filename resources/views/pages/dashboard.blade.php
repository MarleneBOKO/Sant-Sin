                            ...
                            @foreach (array_slice($fakers, 0, 4) as $faker)
                                <tr class="intro-x">
                                    <td class="w-40">
                                        <div class="flex">
                                            @for ($i = 0; $i < 3; $i++)
                                                @if (!empty($faker['images'][$i]) && !empty($faker['dates'][$i]))
                                                    <div class="w-10 h-10 image-fit zoom-in {{ $i > 0 ? '-ml-5' : '' }}">
                                                        <img
                                                            alt="Image {{ $i + 1 }}"
                                                            class="tooltip rounded-full"
                                                            src="{{ asset('dist/images/' . $faker['images'][$i]) }}"
                                                            title="Uploaded at {{ $faker['dates'][$i] }}"
                                                        >
                                                    </div>
                                                @endif
                                            @endfor
                                                                        ...
                                                                        <td>
                                        @if (!empty($faker['products'][0]['name']) && !empty($faker['products'][0]['category']))
                                        ...
                                        @else
                                            <span class="text-gray-400 italic">No product</span>
                                        @endif
                                    </td>

                                    <td class="text-center">
                                        {{ !empty($faker['stocks'][0]) ? $faker['stocks'][0] : 'N/A' }}
                                    </td>

                                    @php
    $status = $faker['true_false'][0] ?? false;
@endphp

<div class="flex items-center justify-center {{ $status ? 'text-theme-9' : 'text-theme-6' }}">
    ...
    {{ $faker['true_false'][0] ? '+' : '-' }}${{ $faker['totals']['orders'] }}
</div>
