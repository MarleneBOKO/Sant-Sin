<div id="traitementModal" class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
  <div class="bg-white rounded-lg p-6 w-full max-w-2xl">
    ...
    <div class="flex justify-between items-center mb-4">
      <h5 class="text-xl font-semibold">Enregistrer la demande</h5>
      <button onclick="closeTraitementModal()" class="text-gray-600 hover:text-gray-900">&times;</button>
    </div>

<form method="POST" action="{{ route('ligne_suivi.traiter',  $facture->Id_Ligne) }}">
    @csrf
    @method('PUT')
...
@endif
      ...
      <div class="mb-4">
        ...
        <input type="text" name="fact" id="fact" readonly class="w-full border px-3 py-2 rounded" />
      </div>

      <div class="mb-4">
        <label>Assuré / Prestataire :</label>
       <input type="text" name="assure" id="assures" readonly class="w-full border px-3 py-2 rounded" />
      ...
      <div class="mb-4 grid grid-cols-2 gap-4">
        <div>
          ...
          <input type="text" id="dateEnreg" readonly class="w-full border px-3 py-2 rounded" />
        </div>
        <div>
          <label>Montant Facture :</label>
          <input type="text" name="MontantF" id="montantF" readonly class="w-full border px-3 py-2 rounded" />
        </div>
      </div>
      ...
      <div class="mb-4 grid grid-cols-2 gap-4">
        <div>
          ...
          <input type="text" name="montrejete" id="montrejete" readonly class="w-full border px-3 py-2 rounded" />
        </div>
        <div>
          <label>Date transmission :</label>
          <input type="text" id="dateTransmission" readonly class="w-full border px-3 py-2 rounded" />
        </div>
      </div>
      ...
      <div class="mb-4 grid grid-cols-2 gap-4">
        <div>
          ...
          <input type="text" name="date_enregg" value="{{ date('d/m/Y') }}" readonly class="w-full border px-3 py-2 rounded" />
        </div>
        <div>
          <label>Saisie effectuée par :</label>
          <input type="text" name="userSaisieReg" value="{{ auth()->user()->name }}" readonly class="w-full border px-3 py-2 rounded" />
        </div>
      </div>
